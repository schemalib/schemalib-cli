...
Services
    = description:Comment* accessType:(Plus / Minus)? "services" SPACE BEGIN_BODY services:(SubGroupList) CLOSE_BODY {
        return {name, nodeType: "services", accessType, description, services};
    }

Action
    = description:Comment* accessType:(Plus / Minus)? name:Keyword SPACE BEGIN_ACTION request:PropertyName? CLOSE_ACTION COLON? response:PropertyName? properties:ExtendedPropertyType? {
        return {name, nodeType: "action", request, response, accessType, description, properties};
    }
